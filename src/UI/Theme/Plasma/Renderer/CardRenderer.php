...
namespace SyntaxDevTeam\MiniPortal\UI\Theme\Plasma\Renderer;
...
final class CardRenderer implements ComponentRenderer
{
    public function render(Component $component, RendererRegistry $registry): string
    {
        if (!$component instanceof Card) {
            throw new \LogicException('Plasma CardRenderer received an unsupported component.');
        }
        $title = $component->title === null ? '' : '<h2 class="mp-card__title">' . Html::escape($component->title) . '</h2>';
        return '<section class="mp-card plasma-card"' . Html::identityAttribute(new ComponentRendererIdentity($component)) . '>'
            . $title . '<div class="mp-card__content">' . $registry->renderMany($component->children()) . '</div></section>';
    }
}
